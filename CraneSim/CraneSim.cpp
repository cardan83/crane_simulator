...
static int shoulderAngle = 0, elbowAngle = 0, rotation = 90;

// Handles the keyboard event: the left and right arrows bend the elbow, the
// up and down keys bend the shoulder.
void special(int key, int, int) {
    switch (key) {
    case GLUT_KEY_LEFT: (elbowAngle += 5) %= 360; break;    // C %= A is equivalent to C = C % A
...
// Dan
void special2(unsigned char key, int, int) {
    switch (key) {
    case 's': (rotation += 5) %= 360; break;
    case 'a': (rotation -= 5) %= 360; break;
    default: return;
    }
    glutPostRedisplay();
}
